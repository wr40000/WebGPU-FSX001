...
//     var elevation = sin(position.x * uBigWavesFrequency.x + time)
//                     * sin(position.z * uBigWavesFrequency.y + time)
//                     * flatElevation;
//     var pos = vec4(
//         position.x,
...
//     let posFromCamera = viewprojectMatrix * modelMatrix * pos;
...
fn main(
    ...
    let posFromCamera = viewprojectMatrix * position_model;
    output.Position = posFromCamera;
    output.fragPosition = modelMatrix * pos;
    output.fragUV = uv;
    output.fragNormal = (modelMatrix * vec4<f32>(normal, 0.0)).xyz;

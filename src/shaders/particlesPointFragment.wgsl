@binding(2) @group(0) var<uniform> lightPosition : vec4<f32>;
...
fn main(
  ...
  @location(2) shadowPos: vec3<f32>,
    ...
    // Directional Light
    let diffuse: f32 = max(dot(normalize(lightPosition.xyz), vec3<f32>(0,1,0)), 0.0);
    // add shadow factor
    var shadow : f32 = 0.0;
    ...
    let lightFactor = min(0.3 + shadow, 1.0);
    return fragColor * lightFactor;
}